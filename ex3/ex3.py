...
def gibbs_generate_x_given_y(num_rows, num_cols, num_iterations):
    ...
    for i in range(0, num_iterations):
        ...
        for row, col in zip(rows, cols):
            ...
            psi_xi_yi_0 = np.sum(-0.5 * (0 - y_grid) ** 2)
            psi_xi_yi_1 = np.sum(-0.5 * (1 - y_grid) ** 2)
...
def correct_vs_gibbs(x_grid, y_grid, num_rows, num_cols, num_iterations):
    ...
    correct_marginals = np.asarray([])
    for row, col in zip(rows, cols):
        location = row * num_cols + col  # get location of cell when flatting the 2d array
        correct_marginals = np.concatenate((correct_marginals, correct_marginal(location, x_grid[row, col], y_grid, num_rows, num_cols)))  # add correct marginal of cell i

    correct_marginals = correct_marginals.reshape((num_rows, num_cols))
    ...
    for idx, gibbs_grid in enumerate(gibbs_grid_array):
        ...
        for row, col in zip(rows, cols):
            if gibbs_grid[row, col] == 0:
                ...
                error += (correct_marginals[row, col] - grid_sum_0[row, col]/idx) ** 2  # (p(x_i=0|y) - p_hat(x_i=0|y))^2
            else:
                grid_sum_1[row, col] += 1
                error += (correct_marginals[row, col] - grid_sum_1[row, col]/idx) ** 2  # (p(x_i=1|y) - p_hat(x_i=1|y))^2
...
if __name__ == '__main__':
    ...
    x_grid = gibbs_generate_x(num_rows, num_cols, num_iterations)
    ...
    correct_vs_gibbs(x_grid, y_grid, num_rows, num_cols, num_iterations)
